...
class ThreeGppPropagationLossModel : public PropagationLossModel
{
  ...
  private:
    void MaybeAppendDebugLog(Ptr<MobilityModel> a,
                             Ptr<MobilityModel> b,
  ...
  protected:
    ...
    bool m_debugLogEnabled;                             //!< enable/disable debug file logging
    double m_debugAssumedTxPowerDbm;                    //!< assumed tx power for debug reporting
    ...
    std::string m_debugLogFileName;                     //!< debug log file path
    mutable bool m_debugLogHeaderWritten;              //!< whether header was already written
    mutable std::unordered_set<uint32_t> m_debugLoggedKeys; //!< logged debug link keys

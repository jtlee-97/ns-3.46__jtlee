...
const std::map<int, double> NTN_DenseUrbanLosProb{
    {10, 0.282}, {20, 0.331}, {30, 0.398}, {40, 0.468}, {50, 0.537},
    {60, 0.612}, {70, 0.738}, {80, 0.820}, {90, 0.981},
};

const std::map<int, double> NTN_UrbanLosProb{
    {10, 0.246}, {20, 0.386}, {30, 0.493}, {40, 0.613}, {50, 0.726},
    {60, 0.805}, {70, 0.919}, {80, 0.968}, {90, 0.992},
};

const std::map<int, double> NTN_SuburbanRuralLosProb{
    {10, 0.782}, {20, 0.869}, {30, 0.919}, {40, 0.929}, {50, 0.935},
    {60, 0.940}, {70, 0.949}, {80, 0.952}, {90, 0.998},
};
...
double
GetNtnLosProbability(const ns3::ChannelConditionModel* channelConditionModel, double elevAngleQuantized)
{
    if (dynamic_cast<const ns3::ThreeGppNTNDenseUrbanChannelConditionModel*>(channelConditionModel))
    {
        return NTN_DenseUrbanLosProb.at(elevAngleQuantized);
    }
    if (dynamic_cast<const ns3::ThreeGppNTNUrbanChannelConditionModel*>(channelConditionModel))
    {
        return NTN_UrbanLosProb.at(elevAngleQuantized);
    }
    if (dynamic_cast<const ns3::ThreeGppNTNSuburbanChannelConditionModel*>(channelConditionModel) ||
        dynamic_cast<const ns3::ThreeGppNTNRuralChannelConditionModel*>(channelConditionModel))
    {
        return NTN_SuburbanRuralLosProb.at(elevAngleQuantized);
    }
...
GetNtnSfclTable(const ns3::ThreeGppPropagationLossModel* lossModel)
{
    if (dynamic_cast<const ns3::ThreeGppNTNDenseUrbanPropagationLossModel*>(lossModel))
    {
        return &SFCL_DenseUrban;
    }
    if (dynamic_cast<const ns3::ThreeGppNTNUrbanPropagationLossModel*>(lossModel))
    {
        return &SFCL_Urban;
    }
    if (dynamic_cast<const ns3::ThreeGppNTNSuburbanPropagationLossModel*>(lossModel) ||
        dynamic_cast<const ns3::ThreeGppNTNRuralPropagationLossModel*>(lossModel))
    {
        return &SFCL_SuburbanRural;
    }
    ...
    static TypeId tid =
        TypeId("ns3::ThreeGppPropagationLossModel")
            ...
            .AddAttribute("ShadowingEnabled",
                          ...
                          MakeBooleanChecker())
            .AddAttribute("DebugLogEnabled",
                          "Enable/disable propagation debug file logging.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&ThreeGppPropagationLossModel::m_debugLogEnabled),
                          MakeBooleanChecker())
            .AddAttribute("DebugLogFileName",
                          "Path to the propagation debug log file.",
                          StringValue(""),
                          MakeStringAccessor(&ThreeGppPropagationLossModel::m_debugLogFileName),
                          MakeStringChecker())
            .AddAttribute("DebugAssumedTxPowerDbm",
                          "Assumed transmitter power used only for debug CSV reporting.",
                          DoubleValue(0.0),
                          MakeDoubleAccessor(&ThreeGppPropagationLossModel::m_debugAssumedTxPowerDbm),
                          MakeDoubleChecker<double>())
            .AddAttribute(
...
ThreeGppPropagationLossModel::ThreeGppPropagationLossModel()
    : PropagationLossModel(),
      ...
      m_debugAssumedTxPowerDbm(0.0),
      m_debugLogHeaderWritten(false)
{
                                            ...
                                            Ptr<MobilityModel> b) const
{
    ...
    const double propagationLossDb = GetLoss(cond, a, b);
    rxPow -= propagationLossDb;

    double shadowingValueDb = 0.0;
    double shadowingStdDb = 0.0;

    if (m_shadowingEnabled)
    {
        shadowingStdDb = GetShadowingStd(a, b, cond->GetLosCondition());
        shadowingValueDb = GetShadowing(a, b, cond->GetLosCondition());
        rxPow -= shadowingValueDb;
    }

    double o2iLossDb = 0.0;
          ...
          cond->GetLosCondition() == ChannelCondition::LosConditionValue::NLOS)))
    {
        ...
        {
            o2iLossDb = GetO2iLowPenetrationLoss(a, b, cond->GetLosCondition());
        }
        else
        {
            o2iLossDb = GetO2iHighPenetrationLoss(a, b, cond->GetLosCondition());
        }
        rxPow -= o2iLossDb;
    }

    MaybeAppendDebugLog(a,
                        b,
                        cond,
                        txPowerDbm,
                        propagationLossDb,
...
void
ThreeGppPropagationLossModel::MaybeAppendDebugLog(Ptr<MobilityModel> a,
                                                  Ptr<MobilityModel> b,
                                                  Ptr<ChannelCondition> cond,
                                                  double txPowerDbm,
                                                  double propagationLossDb,
                                                  double shadowingValueDb,
                                                  double shadowingStdDb,
                                                  double o2iLossDb,
                                                  double rxPowerDbm) const
{
    if (!m_debugLogEnabled || m_debugLogFileName.empty())
    {
        return;
    }

    const Vector txPos = a->GetPosition();
    const Vector rxPos = b->GetPosition();
    const bool txIsSpace = txPos.z > 100000.0;
    const bool rxIsSpace = rxPos.z > 100000.0;
    const bool txIsGround = txPos.z < 10000.0;
    const bool rxIsGround = rxPos.z < 10000.0;
    const bool isSpaceGroundLink = (txIsSpace && rxIsGround) || (txIsGround && rxIsSpace);
    if (!isSpaceGroundLink)
    {
        return;
    }

    const uint32_t key = GetKey(a, b);
    if (m_debugLoggedKeys.contains(key))
    {
        return;
    }

    std::ofstream log(m_debugLogFileName, std::ios::app);
    if (!log.is_open())
    {
        return;
    }

    if (!m_debugLogHeaderWritten)
    {
        log << "timeMs,txNodeId,rxNodeId,model,channelConditionModel,losCond,losProbability,"
               "nlosProbability,selectedConditionProbability,o2iCond,o2iLossDb,freqHz,"
               "distance2D,distance3D,txZ,rxZ,elevationDeg,elevationQuantizedDeg,fsplDb,"
               "clutterDb,atmosAbsDb,scintillationDb,propagationLossDb,shadowingEnabled,"
               "shadowingStdDb,shadowingValueDb,totalLossDb,referenceTxPowerDbm,"
               "referenceRxPowerDbm,actualTxPowerDbm,actualRxPowerDbm"
            << std::endl;
        m_debugLogHeaderWritten = true;
    }

    const auto [elevAngleDeg, elevAngleQuantizedDeg] =
        ThreeGppChannelConditionModel::GetQuantizedElevationAngle(a, b);
    const double distance2D = Calculate2dDistance(a->GetPosition(), b->GetPosition());
    const double distance3D = CalculateDistance(a->GetPosition(), b->GetPosition());
    const auto* sfclTable = GetNtnSfclTable(this);
    const double pLos =
        GetNtnLosProbability(PeekPointer(m_channelConditionModel), elevAngleQuantizedDeg);
    const double pNlos = std::isnan(pLos) ? std::numeric_limits<double>::quiet_NaN() : (1.0 - pLos);

    double fsplDb = std::numeric_limits<double>::quiet_NaN();
    double clutterDb = std::numeric_limits<double>::quiet_NaN();
    double atmosAbsDb = std::numeric_limits<double>::quiet_NaN();
    double scintillationDb = std::numeric_limits<double>::quiet_NaN();

    if (sfclTable)
    {
        fsplDb = ComputeNtnPathloss(m_frequency, distance3D);
        atmosAbsDb = ComputeAtmosphericAbsorptionLoss(m_frequency, elevAngleDeg);
        scintillationDb =
            ComputeIonosphericPlusTroposphericScintillationLoss(m_frequency, elevAngleQuantizedDeg);
        clutterDb = (cond->GetLosCondition() == ChannelCondition::LosConditionValue::NLOS)
                        ? ComputeClutterLoss(m_frequency, sfclTable, elevAngleQuantizedDeg)
                        : 0.0;
    }

    const auto losCond = cond->GetLosCondition();
    const auto o2iCond = cond->GetO2iCondition();
    const double selectedConditionProbability =
        (losCond == ChannelCondition::LosConditionValue::LOS) ? pLos : pNlos;
    const double totalLossDb = txPowerDbm - rxPowerDbm;
    const double actualTxPowerDbm = m_debugAssumedTxPowerDbm;
    const double actualRxPowerDbm = actualTxPowerDbm - totalLossDb;
    const auto txNode = a->GetObject<Node>();
    const auto rxNode = b->GetObject<Node>();

    log << std::fixed << std::setprecision(6)
        << Simulator::Now().GetMilliSeconds() << ','
        << (txNode ? std::to_string(txNode->GetId()) : std::string("na")) << ','
        << (rxNode ? std::to_string(rxNode->GetId()) : std::string("na")) << ','
        << GetInstanceTypeId().GetName() << ','
        << (m_channelConditionModel ? m_channelConditionModel->GetInstanceTypeId().GetName()
                                    : std::string("na"))
        << ',' << losCond << ',' << pLos << ',' << pNlos << ',' << selectedConditionProbability
        << ',';

    if (o2iCond == ChannelCondition::O2iConditionValue::O2O)
    {
        log << "O2O";
    }
    else if (o2iCond == ChannelCondition::O2iConditionValue::O2I)
    {
        log << "O2I";
    }
    else if (o2iCond == ChannelCondition::O2iConditionValue::I2I)
    {
        log << "I2I";
    }
    else
    ...
    log << ',' << o2iLossDb << ',' << m_frequency << ',' << distance2D << ',' << distance3D << ','
        << txPos.z << ',' << rxPos.z << ',' << elevAngleDeg << ',' << elevAngleQuantizedDeg << ','
        << fsplDb << ',' << clutterDb << ',' << atmosAbsDb << ',' << scintillationDb << ','
        << propagationLossDb << ',' << (m_shadowingEnabled ? 1 : 0) << ',' << shadowingStdDb
        << ',' << shadowingValueDb << ',' << totalLossDb << ',' << txPowerDbm << ','
        << rxPowerDbm << ',' << actualTxPowerDbm << ',' << actualRxPowerDbm << std::endl;

    m_debugLoggedKeys.insert(key);
}

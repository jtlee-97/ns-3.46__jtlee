    ...
    static TypeId tid =
        TypeId("ns3::LteEnbMac")
            ...
            .AddAttribute("UseNtnRachTiming",
                          "Use NTN-specific RA timing for local MAC timers while keeping LTE SIB "
                          "ra-ResponseWindowSize ASN encoding unchanged",
                          BooleanValue(false),
                          MakeBooleanAccessor(&LteEnbMac::m_useNtnRachTiming),
                          MakeBooleanChecker())
            .AddAttribute("NtnRaResponseWindowSize",
                          "NTN-specific RA response window size (TTIs) used for local MAC timers "
                          "when UseNtnRachTiming is true",
                          UintegerValue(80),
                          MakeUintegerAccessor(&LteEnbMac::m_ntnRaResponseWindowSize),
        ...
        if ((it == m_allocatedNcRaPreambleMap.end()) || (it->second.expiryTime < Simulator::Now()))
        {
            ...
            const uint16_t effectiveRaResponseWindowSize =
                m_useNtnRachTiming ? m_ntnRaResponseWindowSize
                                   : static_cast<uint16_t>(m_raResponseWindowSize);
            uint32_t expiryIntervalMs =
                (uint32_t)m_preambleTransMax * ((uint32_t)effectiveRaResponseWindowSize + 5);
            NS_LOG_INFO(this << " NC-RA timer window configured: sib="
                             << static_cast<uint32_t>(m_raResponseWindowSize)

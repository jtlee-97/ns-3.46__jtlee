...
LteUeMac::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::LteUeMac")
            ...
            .AddAttribute("UseNtnRachTiming",
                          "Use NTN-specific RA response window for local UE MAC RA timing",
                          BooleanValue(false),
                          MakeBooleanAccessor(&LteUeMac::m_useNtnRachTiming),
                          MakeBooleanChecker())
            .AddAttribute("NtnRaResponseWindowSize",
                          "NTN-specific RA response window size (TTIs) used in UE MAC when "
                          "UseNtnRachTiming is true",
                          UintegerValue(80),
                          MakeUintegerAccessor(&LteUeMac::m_ntnRaResponseWindowSize),
                          MakeUintegerChecker<uint16_t>(2, 10240))
...
LteUeMac::LteUeMac()
    ...
    m_useNtnRachTiming(false),
    m_ntnRaResponseWindowSize(80),
...
LteUeMac::SendRaPreamble(bool contention)
{
    ...
    NS_LOG_INFO(this << " sent preamble id " << (uint32_t)m_raPreambleId << ", RA-RNTI "
                     << (uint32_t)m_raRnti);
    // 3GPP 36.321 5.1.4
    const uint16_t effectiveRaResponseWindowSize =
        m_useNtnRachTiming ? m_ntnRaResponseWindowSize
                           : static_cast<uint16_t>(m_rachConfig.raResponseWindowSize);
    NS_LOG_INFO(this << " RA response window configured: sib="
                     << static_cast<uint32_t>(m_rachConfig.raResponseWindowSize)
                     << " effective=" << effectiveRaResponseWindowSize
                     << " useNtnRachTiming=" << m_useNtnRachTiming);
    Time raWindowBegin = MilliSeconds(3);
    Time raWindowEnd = MilliSeconds(3 + effectiveRaResponseWindowSize);

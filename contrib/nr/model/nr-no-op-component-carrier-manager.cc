    ...
    NrMacSapUser::TxOpportunityParameters txOpParams)
{
    ...
    {
        NS_LOG_WARN("Dropping TxOpportunity for unknown RNTI " << txOpParams.rnti);
        return;
    }

    auto lcidIt = ueIt->second.m_ueAttached.find(txOpParams.lcid);
    if (lcidIt == ueIt->second.m_ueAttached.end() || lcidIt->second == nullptr)
    {
        NS_LOG_WARN("Dropping TxOpportunity for RNTI=" << txOpParams.rnti
                                                        << " missing LCID="
                                                        << static_cast<uint32_t>(txOpParams.lcid));
        return;
    }
...
NrNoOpComponentCarrierManager::DoReceivePdu(NrMacSapUser::ReceivePduParameters rxPduParams)
{
    ...
    if (ueIt == m_ueInfo.end())
    {
        NS_LOG_WARN("Dropping PDU for unknown RNTI " << rxPduParams.rnti);
        return;
    }
...
{
    NS_LOG_FUNCTION(this);
    auto ueIt = m_ueInfo.find(lcInfo.rnti);
    NS_ABORT_MSG_IF(ueIt == m_ueInfo.end(),
                    "Cannot add LC for unknown RNTI " << lcInfo.rnti);
    ueIt->second.m_rlcLcInstantiated.emplace(lcInfo.lcId, lcInfo);
}

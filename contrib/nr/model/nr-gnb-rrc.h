...
class NrUeManager : public Object
{
  ...
  public:
    ...
    /**
     * Prepare handover in conditional mode: complete X2 preparation and store
     * handover command without immediately sending it to UE.
     *
     * @param cellId target cell ID
     */
    void PrepareConditionalHandover(uint16_t cellId);

    /**
     * @return true if a conditional handover command has been prepared and is
     * ready for execution.
     */
    bool IsConditionalHandoverPrepared() const;

    /**
     * @param targetCellId target cell ID
     * @return true if a conditional handover command has been prepared for the target.
     */
    bool IsConditionalHandoverPrepared(uint16_t targetCellId) const;

    /**
     * Execute a previously prepared conditional handover command.
     *
     * @return true on successful execution trigger
     */
    bool ExecutePreparedConditionalHandover();

    /**
     * Execute a previously prepared conditional handover command for target cell.
     *
     * @param targetCellId target cell ID
     * @return true on successful execution trigger
     */
    bool ExecutePreparedConditionalHandover(uint16_t targetCellId);
  ...
  private:
    ...
    bool m_isConditionalHoPreparation;    ///< true while conditional HO preparation is active
    bool m_isConditionalHoPrepared;       ///< true when conditional HO command is prepared
    bool m_hasPendingConditionalCommand;  ///< true when a stored HO command exists
    std::map<uint16_t, NrRrcSap::RrcConnectionReconfiguration>
      m_pendingConditionalHandoverCommands; ///< stored conditional HO command by target cell
    std::map<uint16_t, NrEpcX2SapUser::HandoverRequestAckParams>
      m_pendingConditionalAckParamsByTarget; ///< stored HO ack by target cell
...
class NrGnbRrc : public Object
{
  ...
  public:
    ...
    /**
     * Trigger conditional handover preparation for UE.
     *
     * @param rnti UE RNTI
     * @param targetCellId target cell ID
     * @return true if request accepted
     */
    bool PrepareConditionalHandover(uint16_t rnti, uint16_t targetCellId);

    /**
     * Query whether conditional handover command is prepared for UE.
     *
     * @param rnti UE RNTI
     * @return true if prepared
     */
    bool IsConditionalHandoverPrepared(uint16_t rnti);

    /**
     * Query whether conditional handover command is prepared for UE and target.
     *
     * @param rnti UE RNTI
     * @param targetCellId target cell ID
     * @return true if prepared for target
     */
    bool IsConditionalHandoverPrepared(uint16_t rnti, uint16_t targetCellId);

    /**
     * Execute prepared conditional handover command for UE.
     *
     * @param rnti UE RNTI
     * @return true if execution started
     */
    bool ExecuteConditionalHandover(uint16_t rnti);

    /**
     * Execute prepared conditional handover command for UE and target.
     *
     * @param rnti UE RNTI
     * @param targetCellId target cell ID
     * @return true if execution started
     */
    bool ExecuteConditionalHandover(uint16_t rnti, uint16_t targetCellId);

    ...
    const NrMacSchedSapProvider::SchedUlCqiInfoReqParameters& params)
{
    ...
    case UlCqiInfo::PUSCH: {
        ...
        {
            NS_LOG_WARN("Dropping UL CQI for " << ulSfnSf
                                                << " because the allocation history is missing."
                                                   " This can happen during handover when the"
                                                   " UE moves to a new cell before the old UL"
                                                   " slot state is still available.");
            return;
        }

...
UeRrcProtocolReal::SetGnbRrcSapProvider()
{
    ...
    auto knownGnbIt = m_knownGnb.find(cellId);
    NS_ABORT_MSG_IF(knownGnbIt == m_knownGnb.end() || knownGnbIt->second == nullptr,
                    "Unable to resolve known gNB for cellId=" << cellId
                                                               << " while setting UE RRC SAP");

    m_gnbRrcSapProvider = knownGnbIt->second->GetRrc()->GetNrGnbRrcSapProvider();
    Ptr<NrGnbRrcProtocolReal> gnbRrcProtocolReal =
        knownGnbIt->second->GetRrc()->GetObject<NrGnbRrcProtocolReal>();
...
NrGnbRrcProtocolReal::DoSendRrcConnectionSetup(uint16_t rnti, NrRrcSap::RrcConnectionSetup msg)
{
    ...
    auto setupIt = m_setupUeParametersMap.find(rnti);
    if (setupIt != m_setupUeParametersMap.end() && setupIt->second.srb0SapProvider != nullptr)
    {
        setupIt->second.srb0SapProvider->TransmitPdcpPdu(transmitPdcpPduParameters);
        return;
    }

    NS_LOG_WARN("Missing SRB0 setup for RNTI=" << rnti
                                                << ", falling back to direct RRC delivery");
    auto ueSapIt = m_gnbRrcSapProviderMap.find(rnti);
    NS_ABORT_MSG_IF(ueSapIt == m_gnbRrcSapProviderMap.end() || ueSapIt->second == nullptr,
                    "Cannot send RRC Connection Setup: missing UE SAP provider for RNTI="
                        << rnti);
    Simulator::Schedule(RRC_REAL_MSG_DELAY,
...
NrGnbRrcProtocolReal::DoSendRrcConnectionReject(uint16_t rnti, NrRrcSap::RrcConnectionReject msg)
{
    ...
    NS_LOG_WARN("Missing SRB0 setup for RNTI=" << rnti
                                                << ", falling back to direct reject delivery");
    auto ueSapIt = m_gnbRrcSapProviderMap.find(rnti);
    NS_ABORT_MSG_IF(ueSapIt == m_gnbRrcSapProviderMap.end() || ueSapIt->second == nullptr,
                    "Cannot send RRC Connection Reject: missing UE SAP provider for RNTI="
                        << rnti);
    Simulator::Schedule(RRC_REAL_MSG_DELAY,
